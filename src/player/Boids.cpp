...
#define LOCAL_GOAL_DISTANCE		3		// Detection distance for individual snakes grabbing nearby apples.
...
void Boids::MakeMoves( const GameState currentState, size_t teamIndex, std::vector<Move>& outMoves ) {
	...
	if ( !currentState.IsTileWalkable( m_GoalTile ) || currentState.Board[m_GoalTile.y][m_GoalTile.x] != Tile::Apple ) {
		m_GoalTile		= FindClosestApple( currentState, teamAvaragePosition );		// TODO: Figure out another goal if there are no apples.
	}
